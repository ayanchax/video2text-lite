...
class AudioSplitter:
    def __init__(
        ...
        format="mp3",
        chunk_output_path="output/audio/chunks/",
    ):
        ...
        self.format = format
        utils = CommonUtils()
        self.audio_chunk_name_prefix = (
            f"{utils.get_file_name_without_extension(self.audio_file)}"
        )
        ...
        try:
            ...
            for i in range(num_chunks):
                ...
                chunk_file_name = os.path.join(
                    self.chunk_output_path,
                    f"{self.audio_chunk_name_prefix}_{i+1}.{self.format}",
                )
